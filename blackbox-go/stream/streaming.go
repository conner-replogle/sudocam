...
func Video(options CameraOptions, writer io.Writer) {

	cameraStarted := sync.Mutex{}
	slog.Info("Video: Starting camera")


	go startCamera(options, writer, &cameraStarted)
}

func startCamera(options CameraOptions, writer io.Writer, mutex *sync.Mutex) {
	mutex.Lock()
	defer mutex.Unlock()
