...
ssh $REMOTE_USER@$REMOTE_HOST "pkill -f sudocam && systemctl stop blackbox" 
...
ssh $REMOTE_USER@$REMOTE_HOST "cd $REMOTE_PATH  && chmod +x sudocam && export PION_LOG_DEBUG=all && ./sudocam --config ./dev-config.json"
